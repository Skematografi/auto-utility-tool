...
A matching panel on the **left** side — opened from the floating button in the bottom-left corner — for keeping up to **10** photos nearby while you work: someone you love, a pet, a screenshot, a diagram, whatever you'd normally prop up on your desk. Styled as an Instagram-story-style viewer; each photo keeps its original aspect ratio (never cropped or stretched) and sits in a rounded Polaroid/instax-print frame — a white border around it, thicker along the bottom, with rounded corners on both the photo and its frame. Attach photos by clicking, dragging files onto the panel, or pasting from the clipboard (`Ctrl+V`) — all three accept multiple at once. With more than one photo they auto-shuffle randomly every 5 seconds (pausing while you hover), with a story-style progress bar up top and left/right tap zones for manual navigation; remove the current photo, add more, or clear all from the overlay controls. A fullscreen toggle expands the frame to fill this browser tab (not the device screen) for a closer look — off by default. Unlike the sticky note, none of this is **saved anywhere**: everything lives only in memory for the current page session and disappears on reload.
